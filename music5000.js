...
export function Music5000(onBuffer) {
	...
	this.read = function(page, addr) {
		// Bit0 unused
		var offset = ((page & 0x0e) << 7) + addr;
		return this.waveRam[offset];
	}

	this.write = function(page, addr, value) {
		// Bit0 unused
		var offset = ((page & 0x0e) << 7) + (addr & 0xff);
		this.waveRam[offset] = value;
	}
	
	this.cReg_freqLow = function(channel, regSet) { return this.waveRam[CHANNEL_REG_OFFSET + (regSet * CHANNEL_ROW_SIZE) + (0 * NUM_CHANNELS) + channel]; }
	this.cReg_freqMedium = function(channel, regSet) { return this.waveRam[CHANNEL_REG_OFFSET + (regSet * CHANNEL_ROW_SIZE) + (1 * NUM_CHANNELS) + channel]; }
	this.cReg_freqHigh = function(channel, regSet) { return this.waveRam[CHANNEL_REG_OFFSET + (regSet * CHANNEL_ROW_SIZE) + (2 * NUM_CHANNELS) + channel]; }
	this.cReg_waveformReg = function(channel, regSet) { return this.waveRam[CHANNEL_REG_OFFSET + (regSet * CHANNEL_ROW_SIZE) + (5 * NUM_CHANNELS) + channel]; }
	this.cReg_amplitudeReg = function(channel, regSet) { return this.waveRam[CHANNEL_REG_OFFSET + (regSet * CHANNEL_ROW_SIZE) + (6 * NUM_CHANNELS) + channel]; }
	this.cReg_controlReg = function(channel, regSet) { return this.waveRam[CHANNEL_REG_OFFSET + (regSet * CHANNEL_ROW_SIZE) + (7 * NUM_CHANNELS) + channel]; }
	
	this.waveTableVal = function(table, b) { return this.waveRam[(table * WAVE_TABLE_SIZE) + b]; }
	
	this.polltime = function(cycles) {
		var c4d, freq, offset, wavetable, amplitude, control, data, sign, pos = 0 >>> 0;
		...
		while (this.cycleCount >= 8)
		{
			...
			{
				freq = (this.cReg_freqHigh(this.curCh, this.activeRegSet) << 16)
			...
			if (this.curCh == NUM_CHANNELS)	{
				...
				// Range check
				if (this.sampleLeft < -32768)
					this.sampleLeft = -32768;
				else if (this.sampleLeft > 32767)
					this.sampleLeft = 32767;

				if (this.sampleRight < -32768)
					this.sampleRight = -32768;
				else if (this.sampleRight > 32767)
					this.sampleRight = 32767;
